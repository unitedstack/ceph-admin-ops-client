            ...
            return new Promise(function(resolve, reject) {
                    ...
                    Object.getOwnPropertyNames(givenParameters).forEach(function(parameter) {
                        if (allowedParametersArray.indexOf(parameter) > -1) {
                            ...
                            if (methodDefinition.allowedParameters[parameter] !== "date") {
                                if (typeof parameter !== methodDefinition.allowedParameters[parameter] ) {
                                    errorObj[parameter] = "Expected " + methodDefinition.allowedParameters[parameter] + " but got " + (typeof parameter);
                                }
                            } else {
                ...
                request(requestOptions).then(function(response) {
                    if (response.statusCode === 200 || response.statusCode === 201) {
                        ...
                        reject({ message: "Error: Ceph Object Gateway returned non-ok status code: " + response.statusCode });
                    }
